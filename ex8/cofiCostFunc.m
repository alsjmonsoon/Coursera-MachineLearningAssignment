...
% Compute the movie rating error by subtracting Y from the predicted ratings
        error=predicted_rating-Y;

% Compute the "error_factor" my multiplying the movie rating error by the R matrix
        error_factor=error.*R;

% calculate the cost Funciton
        J= (1/2)*sum (sum(error_factor.^2));

% calcuate the X gradient. The sum is computed automatically by the vector multiplication.
% Dimensions are (movies x features).
        X_grad=error_factor*Theta;

% calucate the Theta_grad.  A transposition may be needed. The sum is computed automatically by the vector % multiplication. Dimensions are (users x features): 
